...
export default function UserMenu({ rounded = "rounded-xl" }) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button type="button" aria-label="사용자 메뉴" aria-expanded={open} onClick={() => setOpen((value) => !value)} className={`flex h-9 w-9 items-center justify-center ${rounded} border border-[#7C3AED]/40 bg-[#7C3AED]/30 text-xs font-bold text-[#C084FC] transition-transform active:scale-95`} style={{ fontFamily: "'Outfit', sans-serif" }}>
        {(user.name || "?")[0]}
      </button>
      {open && (
